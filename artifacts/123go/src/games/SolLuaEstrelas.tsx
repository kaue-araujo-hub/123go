...
const PHASES = [
  ...
  },
  {
    ...
    period: 0,
    actIdx: 0,
    question: "O que se faz de MANHÃ?",
  },
...
export function SolLuaEstrelas() {
  ...
  const handlePeriodTap = (periodKey: string) => {
    ...
    const takenBy = Object.entries(connections).find(
      ([, pk]) => pk === periodKey,
    );
    if (takenBy) return;
        ...
        >
          ...
          >
            {shuffledMeals.map((meal) => {
              ...
              const connColor = isConnected
                ? LINE_COLORS[
                    Object.keys(connections).indexOf(meal.key) %
                      LINE_COLORS.length
                  ]
                : undefined;
              return (
                ...
                >
                  <AppleEmoji emoji={meal.emoji} size={isDesktop ? 12 : 14} />
                  <span
                    style={{
                      fontFamily: "Nunito",
                      fontWeight: 700,
          ...
          >
            {MATCH_PERIODS.map((period) => {
              const connEntry = Object.entries(connections).find(
                ([, pk]) => pk === period.key,
              );
              const isConnected = connEntry !== undefined;
              const connColor = isConnected
                ? LINE_COLORS[
                    Object.keys(connections).indexOf(connEntry![0]) %
                      LINE_COLORS.length
                  ]
                : undefined;
  ...
  /* ── Activity phase (4) ── */
  if (
    phaseData.type === "activity" &&
    phaseData.period !== undefined &&
    phaseData.actIdx !== undefined
  ) {
        ...
        >
          {options.map((act, i) => (
            <button
              ...
              onPointerUp={() =>
                handleAnswer(
                  act.label === correct.label ? (phaseData.correct ?? 0) : -1,
                )
              }
              style={{
                padding: "16px 14px",
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <AppleEmoji
          emoji={phaseData.showEmoji ?? "☀️"}
          size={isDesktop ? 56 : 88}
        />
      </div>
        ...
        {PERIODS.map((p, i) => (
          <button
            ...
            style={{
              ...
              background:
                i === (phaseData.correct ?? -1) && answered ? "#E8F5E9" : p.bg,
              display: "flex",
              alignItems: "center",
              gap: 10,
              cursor: "pointer",
