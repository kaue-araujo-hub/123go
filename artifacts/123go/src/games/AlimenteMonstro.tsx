...
export function AlimenteMonstro() {
  ...
  useEffect(() => {
    ...
    setPhase3Ok(false);
    setItems(
      Array.from({ length: phaseData.total }, (_, i) => ({
        id: i,
        eaten: false,
      })),
    );
  ...
  const itemSize = isDesktop ? 22 : 48;
  ...
  function handleFeedItem(itemId: number) {
    ...
    const newFed = fed + 1;
    setItems((prev) =>
      prev.map((it) => (it.id === itemId ? { ...it, eaten: true } : it)),
    );
    setFed(newFed);
  ...
  const startDrag = useCallback(
    (e: React.PointerEvent, itemId: number) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          O monstro quer{" "}
          <strong style={{ color: "#FF6B35", fontSize: 22 }}>
            {phaseData.pedido}
          </strong>{" "}
      ...
      >
        ...
        {overflow && <span style={{ fontSize: 18, marginLeft: 4 }}>⚠️</span>}
      </div>
      ...
      >
        {items.map((item) => (
          <div
            ...
            style={{
              fontSize: itemSize,
              cursor: item.eaten ? "default" : "grab",
